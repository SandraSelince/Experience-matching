...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-white text-sm font-bold">EM</div>
            <span className="font-bold text-gray-900 text-lg">Experience Matching</span>
          </div>
          <nav className="hidden md:flex items-center gap-6 text-sm text-gray-600">
            <a href="#how-it-works" className="hover:text-violet-600 transition-colors">{l.howTitle.split("?")[0]}</a>
            <a href="#candidates" className="hover:text-violet-600 transition-colors">{t.nav.candidates}</a>
            <a href="#companies" className="hover:text-violet-600 transition-colors">{t.nav.companies}</a>
          </nav>
          <div className="flex items-center gap-2">
            <LangToggle />
      ...
      {/* Hero */}
      <section className="relative overflow-hidden bg-gradient-to-br from-violet-50 via-white to-indigo-50 pt-24 pb-32">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-40 -right-40 w-96 h-96 bg-violet-200 rounded-full opacity-20 blur-3xl" />
          <div className="absolute -bottom-20 -left-20 w-80 h-80 bg-indigo-200 rounded-full opacity-20 blur-3xl" />
        </div>
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          <Badge className="mb-6 bg-violet-100 text-violet-700 border-0 px-4 py-1.5 text-sm">{l.badge}</Badge>
            ...
            <div id="companies">
              ...
              <div className="space-y-6">
                {l.managerSteps.map((item, i) => (
                  <div key={item.title} className="flex gap-4">
                    <div className="flex-shrink-0 w-8 h-8 rounded-full bg-indigo-50 border-2 border-indigo-200 flex items-center justify-center text-xs font-bold text-indigo-600">
                      {String(i + 1).padStart(2, "0")}
                    </div>
                    <div>
                      <div className="font-semibold text-gray-900 mb-1">{item.title}</div>
      ...
      <section className="py-16 bg-gradient-to-br from-violet-600 to-indigo-700 text-white">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-violet-200 mb-10 max-w-2xl mx-auto">{l.trustSub}</p>
            ...
            {[{ icon: "🔒", ...l.trust[0] }, { icon: "🎓", ...l.trust[1] }, { icon: "✋", ...l.trust[2] }, { icon: "🎯", ...l.trust[3] }].map((item) => (
              <Card key={item.title} className="bg-white/10 border-white/20 text-white">
                <CardContent className="pt-6 text-center">
                  ...
                  <div className="text-xs text-violet-200">{item.desc}</div>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <footer className="border-t border-gray-100 py-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between text-sm text-gray-500">
          <div className="flex items-center gap-2 mb-4 md:mb-0">
            <div className="w-6 h-6 rounded bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center text-white text-xs font-bold">EM</div>
